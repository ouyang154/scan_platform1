          ...
          <a-row :gutter="16">
            ...
            <a-col :md="8" :sm="24">
              <a-form-model-item label="导入">
                <a-button type="primary" style="width: 45%" @click="importSpecimenInfo">本平台</a-button>
                <a-button type="default" style="width: 45%;margin-left: 5px;" @click="importSpecimenInfoFromLIS">LIS</a-button>
              </a-form-model-item>
            </a-col>
              ...
              <a-col :span="12">
                <a-col :md="24" :sm="24">
                  <a-form-model-item :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }" ref="AIdiagnosis" label="AI结果">
                    ...
                    <!-- <a-button class="toggle" :class="{active:AI_unsatisfied}" @click="AI_unsatisfied=!AI_unsatisfied">结果不满意</a-button> -->
                  </a-form-model-item>
                </a-col>
                <!-- / AI结果 -->
...
import { updateScan, getSpecimenList, getSpecimenInfoFromLIS } from '@/api/workspace'
...
export default {
  ...
  methods: {
    ...
    edit (record) {
      ...
      const {
        AIdiagnosis,AIgrade,status,scan_folder
      } = _.cloneDeep(record)
      this.form_scan = {
        AIdiagnosis:AIdiagnosis,status:status,scan_folder:scan_folder,AIgrade:AIgrade
      }
      this.AIdiagnosis = this.form_scan.AIdiagnosis
      this.id = record.id
    },
    date_format(date){
    ...
    async handleConfirm () {
      if(this.form.name){
        try {
          ...
          let {
            AIdiagnosis,status, AIgrade
          } = _.cloneDeep(this.form_scan)
          if (AIdiagnosis !=this.AIdiagnosis){
            if (AIdiagnosis==1){ AIgrade = 'II'}
            else {AIgrade = 'I'}          
          }
          const parameter = {
      ...
      if(this.form.specimen_id){
        ...
        .then(res=>{
          if (res.count>0){
            ...
            this.$message.success('成功读取病例信息（本平台）')
          }
          else{
    ...
    importSpecimenInfoFromLIS(){
      if(this.form.specimen_id){
        const parameter = {
          specimen_id: this.form.specimen_id
        }
        getSpecimenInfoFromLIS(parameter).then(res => {
          if (res.code === 0) {
            this.form = res.patient_info
            this.$message.success('已加载LIS信息')
          } else if (res.code === 1) {
            // 查无此人
            this.$message.error('LIS查无此人！message:' + res.message)
          } else {
            this.$message.error('未知错误！请检查LIS接口设置! message:' + res.message)
          }
        }).catch(function(error) {
          console.log('LIS接口返回错误', error)
          this.$message.error('无法连接LIS，请检查LIS接口设置！')
        }.bind(this))
